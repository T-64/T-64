# 🏆 Awards & Honors

> Competitive ML track record — every number below is verifiable at the linked leaderboard.

## 🥇 Kaggle

| | Competition | Rank | When | Highlights |
|---|---|---|---|---|
| 🥈 **Silver** | [Stanford RNA 3D Folding](https://www.kaggle.com/competitions/stanford-rna-3d-folding/leaderboard) | **49 / 1516** (Top 3%) | 2025.02 | Geometric deep learning on 3D molecular structure; heavy ensembling under strict runtime limits |
| 🥉 **Bronze** | [UM Game-Playing Strength of MCTS variants](https://www.kaggle.com/competitions/um-game-playing-strength-of-mcts-variants/leaderboard) | **101 / 1680** (Top 6%) | 2024.12 | Predicting game-playing strength from MCTS traces — evaluation design mattered more than model size |
| 🏅 Top 1.6% | [DRW Crypto Market Prediction](https://www.kaggle.com/competitions/drw-crypto-market-prediction/leaderboard) | **18 / 1091** | 2025.04 | Noisy financial time series; feature engineering + validation discipline |

## 🏭 Industry Contests

| | Contest | Rank | When | Notes |
|---|---|---|---|---|
| 🥈 **2nd Place** | [Trillion-Token Corpus Dedup — Alibaba Cloud × NVIDIA Data+AI Engineer Global Challenge](https://tianchi.aliyun.com/competition/entrance/532448) | **2 / 1028** teams | 2026.03 | Top 0.2% · RMB 20k+ cash prize |
| 🏅 **6th Place** | [AI Agent Construction track — same challenge](https://tianchi.aliyun.com/competition/entrance/532448) | 6 / 1028 teams | 2026.03 | LLM-Agent engineering track |
| 🏅 **Top 2%** | [Tencent Advertising Algorithm Contest](https://algo.qq.com/2025) | **29 / 1334** | 2025.10 | Real-scale ads conversion/ranking problem, judged by Tencent AMS engineers |

## 🎓 Academic

- 🎖 **Academic Scholarship** — Beijing Jiaotong University, 2023
- 📄 **First-author research** — *Unleashing the Power of RGB-Opacity of Gaussians for Dense RGB-D SLAM*
  - State-of-the-art rendering quality on **TUM-RGBD / Replica / ScanNet**
  - Proposed the RGB-Opacity attribute, RGB-driven Gaussian expansion, and an edge loss for low-texture regions
- 🌏 **IELTS 7.5** (Listening 8.0 / Reading 8.0)

## 🧰 What the grind looked like

- 4 Kaggle competitions, 2 medals, all solo runs — evenings and weekends, on a single GPU
- Alibaba Cloud × NVIDIA "Data+AI" Global Challenge: 2nd & 6th of 1,028 teams across two tracks
- Every solution reproducible: fixed seeds, versioned features, written post-mortems
- Favorite lesson from silver: *when everyone has the same model, the edge is in the data pipeline*
